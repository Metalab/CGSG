...
void MySDLVU::DrawPoly(const Polygon &poly, float height) {
  if (poly.size() < 3) return;
	...
	if(spectrumIndex>(MySpectrumsize/2) || spectrumIndex > polygons->size()) {
		spectrumIndex=0;
	}
//	glColor3f(1., 0, (GLfloat)MySpectrum[spectrumIndex]);
  ...
  for (int i=poly.size()-1, n; i >= 0; i--) {
    ...
    glVertex3f(poly[i].x+(int)MySpectrum[spectrumIndex]%5, -poly[i].y+(int)MySpectrum[spectrumIndex]%5, height+(MySpectrum[spectrumIndex]/2));
    glVertex3f(poly[n].x+(int)MySpectrum[spectrumIndex]%5, -poly[n].y+(int)MySpectrum[spectrumIndex]%5, height+(MySpectrum[spectrumIndex]/2));
