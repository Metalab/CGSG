...
{
  // Mathematically,  Quat(a->b) = sqrt(Normalize(Quat(a dot b, a cross b)))
  // Where sqrt is defined as for complex numbers -- the quat obtained by
  // dividing the Arg angle in half.
#if 1

  // A unit quat is q=( cos(theta/2),  v sin(theta/2) )
  //    sqrt(Normalize(Quat(a dot b, a cross b)))
  //  = sqrt(Normalize((|a||b|cos(theta), |a||b| v sin(theta)) )
  //  = sqrt( (cos(theta), v sin(theta) ) == sqrt( (s1, v1) )
  //  = (  cos(acos( s1 )/2) , v sin(Acos(s1)/2) )
  Type M = a.LengthSqr()*b.LengthSqr();

  if (M<FUDGE())  // one of the vectors is near-zero magnitude
  { *this = Quatf::IDENTITY(); return *this; }

  M = Type(1.0)/Sqrt(M);
  s = a*b; // dot product |s| = |a||b|cos(theta)
  s *= M;  // |s| = cos(theta)

  if (Type(1.0)-s < FUDGE()) // the two vectors were nearly identical
  { *this = Quatf::IDENTITY(); return *this; }

  v = a^b; // cross product |v|=|a||b|sin(theta)
  v *= M;  // now |v|=sin(theta)

  // Use half angle formula: cos^2(theta/2)=(1+cos(theta))/2
  Type c2 = (1+s)*Type(0.5);
  s = Sqrt(c2);
  v *= Sqrt((1-c2)/v.LengthSqr()); // *see note
  return *this;

  // *note: I also tried using this variation of half angle formula for sin:
  //    sin(th/2)/sin(th) = 1/(2*cos(th/2))
  // which makes the line
  //    v *= Sqrt((1-c2)/v.LengthSqr());
  // become just
  //    v /= 2*s;
  // Unfortunately, this suffers from precision problems, and actually
  // didn't give that much of an additional speedup.

#else
  // This old code runs in 300% the time of the above code!
  // (and is a lot harder to read!)
  qvec u1(a);
  qvec u2(b);
...
template <class Type>
inline Type Quat<Type>::Sqrt(double d)  { return Type(sqrt(d)); }
template <class Type>
